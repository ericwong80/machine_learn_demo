...
save_figure(fig1, "dt_实际值vs预测值.png", output_dir)
plt.close(fig1)
...
save_figure(fig2, "dt_残差图.png", output_dir)
plt.close(fig2)
...
fig3.suptitle("决策树结构", fontsize=16, fontweight='bold')
save_figure(fig3, "dt_决策树结构.png", output_dir)
plt.close(fig3)
...
print("\n=== 保存结果 ===")
# 保存指标
save_metrics(test_metrics, "dt_测试集指标.json", output_dir)

# 保存特征重要性
importance_df.to_csv(os.path.join(output_dir, "dt_特征重要性.csv"), index=False, encoding='utf-8-sig')
...
new_pred_df.to_csv(os.path.join(output_dir, "dt_新样本预测.csv"), index=False, encoding='utf-8-sig')

# 保存决策树结构信息
tree_info = {
    "深度": model.get_depth(),
    "叶子节点数": model.get_n_leaves(),
    "节点数": model.tree_.node_count,
    "特征数量": model.n_features_in_,
    "特征名称": feature_names
}
import json
with open(os.path.join(output_dir, "dt_决策树信息.json"), 'w', encoding='utf-8') as f:
    json.dump(tree_info, f, ensure_ascii=False, indent=4)
